...
void GPIOB_C_init()
{
	...
	GPIOB_MODE  &= (0xC3FFFFFF);//clearing the [27,26] positions
	GPIOB_MODE  |= (0x14000000);//loading 01 to set port-B as output
...
int main()
{
	...
	while(1)
	{
		...
		{
			flag1=0;
			GPIOB_ODR &=~(0x1<<13); //ON RED LED
		}
		else if(flag2==1)
		{
			flag2=0;
			GPIOB_ODR &=~(0x1<<14); //ON green LED
		}
	}
}
...
{
	if ((EXTI_PR	&=(0x1<<8)))//checking PR bit position 10
	{
		flag1=1; 
		cn++;
		EXTI_PR |=(0x1<<8); // clear PR bit position 10
		
	}
	if ((EXTI_PR	&=(0x1<<9)))//checking PR bit position 10
	{
		flag2=1; 
		cn1++;
		EXTI_PR |=(0x1<<9); // clear PR bit position 10
}
	}
